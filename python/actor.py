...
class MeshBlockActor:
    ...
    def update_ghost(self, offsets: (int, int, int)) -> [ObjectRef]:
        ...
        if len(nbs) > 1:  # neighbors at finer level
            tasks = [nb.get_restrict.remote(nb_offsets, self.mblock.coord) for nb in nbs]
        elif self.level_diff(ray.get(nbs[0].get_logicloc.remote())) > 0:  # neighbor at coarser level
...
# This is not supported by Ray yet.
#@ray.remote
