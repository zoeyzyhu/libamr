...
def test_part_function():
    ...
    rs = me.RegionSize(x1dim=(0, 120., 8), x2dim=(0, 120., 8))
    root = me.Tree(rs)
    root.create_tree()
    root.print_tree()

    coarse_node = root.leaf[0]
    print(f"\ncoarse_node = {coarse_node}")
    coarse_block = me.MeshBlock(coarse_node.size, "cartesian", nghost=2)
    coarse_block.allocate().fill_random()
    coarse_block.print_data()
    print("\n===== split block =====")
    root.leaf[1].split_block()
    fine_node1 = root.leaf[1].leaf[0]
    fine_node2 = root.leaf[1].leaf[1]
    print(f"\nfine_node1 = {fine_node1}")
    fine_block1 = me.MeshBlock(fine_node1.size, "cartesian", nghost=2)
    fine_block1.allocate().fill_random()
    fine_block1.print_data()
    print(f"\nfine_node2 = {fine_node2}")
    fine_block2 = me.MeshBlock(fine_node2.size, "cartesian", nghost=2)
