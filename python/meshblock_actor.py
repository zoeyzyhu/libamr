...
class MeshBlockActor:
    ...
    def get_view(self, offset: (int, int, int), lx1: int = 0, lx2: int = 0, lx3: int =
                 0, method: str = "") -> np.ndarray:
    ...
    def update_ghost(self, offset: (int, int, int)) -> None:
        ...
        if len(neighbors) > 1: # neighbors at finer level
            tasks = [neighbor.get_view.remote(-offset, neighbor.lx1, neighbor.lx2,
                                              neighbor.lx3, "prolongation") for neighbor in neighbors]
        elif neighbors[0].level < self.mblock.level: # neighbors at coarser level
            tasks = [neighbor.get_view.remote(-offset, neighbor.lx1, neighbor.lx2,
                                              neighbor.lx3, "restriction") for neighbor in neighbors]
        else: # neighbors at same level
            tasks = [neighbor.get_view.remote(-offset) for neighbor in neighbors]
