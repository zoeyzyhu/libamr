# ------------------------------------------------
# 1. Initiate mesh blocks with geometric info
# 2. Set memory views for interior zones
# 3. Fill ghost zones of a specific position
# ------------------------------------------------
...
class MeshActor:
	...
	def set_mview(self):
		for o1 in range(-1, 2):
			for o2 in range(-1, 2): 
				if o1 == -1: offset1, len1 = self.NGHOST, self.NGHOST
				if o1 == 0: offset1, len1 = self.NGHOST, self.nx1
				if o1 == 1: offset1, len1 = self.nx1, self.NGHOST
				if o2 == -1: offset2, len2 = self.NGHOST, self.NGHOST
				if o2 == 0: offset2, len2 = self.NGHOST, self.nx2
				if o2 == 1: offset2, len2 = self.nx2, self.NGHOST
	...
	def get_ghost_dimensions(self, cubic_offsets):
		o2, o1 = cubic_offsets
		if o1 == -1: offset1, len1 = 0, self.NGHOST
		if o1 == 0: offset1, len1 = self.NGHOST, self.nx1
		if o1 == 1: offset1, len1 = self.NGHOST + self.nx1, self.NGHOST
		if o2 == -1: offset2, len2 = 0, self.NGHOST
		if o2 == 0: offset2, len2 = self.NGHOST, self.nx2
		if o2 == 1: offset2, len2 = self.NGHOST + self.nx2, self.NGHOST
	...
	def update_with_mview(self, neighbor, my_cubic_offsets):
		...
		ready_refs, remain_refs = ray.wait([neighbor.get_mview.remote((-o2, -o1))])
...
def find_meshblock(x1: float, x2: float, meshblock_limits, actors):
	for i, limit in enumerate(meshblock_limits):
		if (x1 > limit[0] and x1 < limit[2] and 
			x2 > limit[1] and x2 < limit[3]):
			return actors[i]
		else:
			pass # currently ignore if it's on border
...
for i, volume in enumerate(volumes):
	x1v, x2v = volume
	if x1v > 0 and x2v > 0 and x1v < mesh1_max and x2v < mesh2_max:
...
for i in range(len(owners)):
	...
	print(f"\n ===== The data owner # {i} ===== ")
	print_actor_info(owners[i])

# Fill ghose zones
for i in range(len(owners)):
	ray.get(consumers[i].update_with_mview.remote(owners[i], coffset))
