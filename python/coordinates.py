...
class Coordinates:
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, Coordinates):
            return self.x1f == other.x1f and self.x2f == other.x2f and self.x3f == other.x3f and self.nghost == other.nghost
        return False
...
class Cartesian(Coordinates):
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, Cartesian):
            return super().__eq__(other) and self.x1v == other.x1v and self.x2v == other.x2v and self.x3v == other.x3v
        return False
...
class Cylindrical(Coordinates):
    def __init__(self, rs: RegionSize, nghost: int = 0):
        super().__init__(rs, nghost)
        if (rs.x1min < 0):
            raise ValueError("x1min (minimum radius) must be >= 0 for cylindrical coordinates")

        self.x1v = 2. / 3. * (pow(self.x1f[1:], 3) - pow(self.x1f[:-1], 3)) / (pow(self.x1f[1:], 2) - pow(self.x1f[:-1], 2))
        self.x2v = (self.x2f[1:] + self.x2f[:-1]) / 2.0
        self.x3v = (self.x3f[1:] + self.x3f[:-1]) / 2.0
